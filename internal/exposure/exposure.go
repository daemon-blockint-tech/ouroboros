...
import (
	...
	"github.com/daemon-blockint-tech/ouroboros/internal/match"
	"github.com/daemon-blockint-tech/ouroboros/internal/model"
	"github.com/daemon-blockint-tech/ouroboros/internal/normalize"
)
...
type Catalog struct {
	...
	// SemverMatch enables constraint-style version matching (>=, <, etc.)
	// in addition to exact string equality. See internal/match.
	SemverMatch bool
...
func (c *Catalog) MatchAll(r model.Record) []Match {
	...
	for _, e := range hits {
		if c.SemverMatch {
			if mv, ok := match.VersionMatches(e.Versions, r.Version); ok {
				out = append(out, Match{Entry: e, Version: mv})
			}
			continue
		}
