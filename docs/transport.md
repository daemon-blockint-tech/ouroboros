...
`ouroboros` writes NDJSON to one of three sinks:
...
```sh
ouroboros scan --profile deep --root "$HOME" > inventory.ndjson
```
...
ouroboros scan \
  --profile baseline \
  --output file \
  --output-file /var/log/ouroboros/inventory.ndjson \
  --append
```
...
ouroboros scan \
  ...
  --http-auth bearer \
  --http-token-env OUROBOROS_TOKEN \
  --http-gzip \
  --device-id-env OUROBOROS_DEVICE_ID
```
...
- Header: `User-Agent: ouroboros/<scanner_version>` when available
