...
func TestResolveDeviceIDFromEnv(t *testing.T) {
	t.Setenv("OUROBOROS_TEST_DEVICE_ID", "  abc-123  ")
	id, warn := resolveDeviceID("OUROBOROS_TEST_DEVICE_ID")
	if id != "abc-123" {
...
func TestResolveDeviceIDMissingEnv(t *testing.T) {
	const name = "OUROBOROS_TEST_DEVICE_ID_MISSING"
...
func TestResolveDeviceIDEmptyEnv(t *testing.T) {
	t.Setenv("OUROBOROS_TEST_DEVICE_ID_EMPTY", "   ")
	id, warn := resolveDeviceID("OUROBOROS_TEST_DEVICE_ID_EMPTY")
	if id != "" {
		t.Fatalf("id = %q, want empty", id)
	}
...
// It returns the absolute parent path (OUROBOROS_USERS_DIR value) and
...
func TestResolveRootsBaselineAllUsersExpansion(t *testing.T) {
	...
	t.Setenv("OUROBOROS_USERS_DIR", usersDir)
...
func TestVersionStringShape(t *testing.T) {
	s := versionString()
	if !strings.HasPrefix(s, "ouroboros ") {
		t.Errorf("versionString = %q, want 'ouroboros ' prefix", s)
	}
